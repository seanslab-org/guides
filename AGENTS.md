# sugr — Engineering Guidelines

This project follows the seanslab Engineering Guideline. The source of truth is the GitHub repo `seanslab-org/guides` (cloned on this Mac at `~/seanslab/seanslab/guides`); a verbatim mirror of it sits next to this file as `Engineering Guide.md` (synced 2026-09-03 from commit `d53cab0`). `AGENTS.md` and `CLAUDE.md` are identical sugr-specific instruction files for Codex and Claude Code. They carry every rule from the guide plus the project header below and sugr-only additions (the `~/sugr` root, the §9 ID-collision rule, and the standing design direction). Keep both files identical when updating either one. When the guide gains a new rule, merge it into both files; do not overwrite them wholesale, or the sugr-only parts are lost.

The standard is **"firmware bring-up discipline"**: plan first, instrument the work, verify each stage, and only then declare completion.

**Project:** Sugr — "Plaud without cloud": a recorder card per person + a compute box in the office/home + our app. Plaud-class meeting notes with total data ownership, no cloud fee, and optional use of the customer's own LLM service.

**Philosophy:** Less structure, more intelligence — we always follow this. Before adding anything a person must build or keep up (a folder, a tag, a setting, a field, a template, a process step), ask whether the machine could work it out instead; if it could, build that. Applies to the product and to how we work. Full statement: `docs/prd-software.md` §3.

---

## Standing design direction

Sean's instruction, 7 September 2026: **For design tasks, always design iOS and Mac with the same design language.**

- Start from the existing approved screens and components on both platforms. Preserve shared typography, colors, icon vocabulary, control shapes, spacing, and visual hierarchy.
- Carry approved details across platforms rather than independently redesigning the companion interface. Adapt only where screen size or platform interaction requires it, such as hover to tap, larger touch targets, or responsive arrangement.
- For a feature-specific proposal, change only that feature and preserve the surrounding approved interface. Do not reconstruct or restyle the whole screen and present it as unchanged.
- A request targeting one platform does not require unsolicited work on the other, but its design must remain consistent with the shared language and approved counterpart.

---

## 0) Workspace and Project Structure

1. **Root work folder (authoritative):** `~/sugr` — the only place the project lives *(moved out of `~/seanslab/` on 2026-08-30; the leftover `~/seanslab/sugr` symlink was deleted on 2026-08-31, so old paths no longer resolve)*
2. **New project setup:**
   - Create a new folder under `~/seanslab/`
   - Initialize git
...
2. **Visible progress**
   ...
   - For big tasks (e.g. downloading big files, compiling a big source tree), report at every 15%; keep the user updated on the progress.
...
1. **Log every user requirement in `Requirements.md`**
   ...
   - **Re-read the top of `Requirements.md` at write time to pick the ID.** Numbers race between parallel sessions — two R-022s were written on 2026-08-30. If you find a collision, the later writer yields and renumbers.
